...
* Versione 1.2 05-09-2011
  Inserimento controllo lunghezzi campi restituiti:
   sc_get_date / c_get_time

*/

var versioneJS = "1.2"
var s_account=temp_reportsuite

var s=s_gi(s_account)
// s.dynamicAccountSelection=true
// s.dynamicAccountList="chebancaprod=www.chebanca.it"
/************************** CONFIG SECTION **************************/
...
s.linkInternalFilters="javascript:,chebanca.it"
...
/*
 * Funzioni per variabili primo tracciato
 */

var data = new Date()
// Get date Che Banca nel formato 	DATE (YYYYMMDD)
function sc_get_date(){
     ...
     giorno = data.getDate()+""
     mese.length == 2 ? datafull+=mese : datafull+="0"+mese;
     giorno.length == 2 ? datafull+=giorno : datafull+="0"+giorno;
     
     if(datafull.length != 8)
       datafull = "19000101"
...
// Get date Che Banca nel formato 	TIME (HH24MISS)
function sc_get_time(){
     ...
     hh24.length == 2 ?  timefull=hh24 : timefull="0"+hh24
     mi.length == 2 ?  timefull+=mi : timefull+="0"+mi
     ss.length == 2 ?  timefull+=ss : timefull+="0"+ss
     
     if(timefull.length != 6)
      timefull = "000000"
...
function sc_createCookie(name,value,mins) {
	if (mins) {
		var date = new Date();
...
function sc_eraseCookie(name) {
	createCookie(name,"",-1);
}

function sc_pageCount (cookie_name)
{
 if (cookie_name == null){
  cookie_name = "sc_pageCount";
  }

 var count = sc_readCookie(cookie_name),
  value = 1;
 ...
 sc_createCookie(cookie_name, value, 30);
 
 return value;
}

function sc_visitID (cookie_name)
{
 if (cookie_name == null){
  cookie_name = "sc_visitID";
  }
 var count = sc_readCookie(cookie_name),
  value = Math.random()*1000000000000000000000;
...
/*
 *  Fine Funzioni per primo tracciato
 */



 s.prop41 = versioneJS; //Versione JS
 s.prop42="D=s_vi" // captures the s_vi (visitor ID) cookie value in prop42
 
 s.prop43 = sc_visitID() // Visit ID 
 s.prop44 = sc_pageCount() // Page Rank 

 s.prop45 = sc_get_date() // DATE (YYYYMMDD)
 s.prop46 = sc_get_time() // TIME (HH24MISS)
...
s.visitorNamespace="chebanca"
s.trackingServer="metrics.chebanca.it"
s.trackingServerSecure="smetrics.chebanca.it"
s.dc=112
